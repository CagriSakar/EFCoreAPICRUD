...
app.MapGet("/", () => "Welcome to the Cagri's DB");
...
app.MapDelete ("/superhero/{id}", async (DataContext context, SuperHero hero, int id) =>
{
    var dbHero = await context.SuperHeroes.FindAsync(id);
    if (dbHero == null) return Results.NotFound("Who's the hero?");
